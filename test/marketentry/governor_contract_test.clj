  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Corozal Trading Co. Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Corozal Trading Co. Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest struck-off-is-held-and-unoverridable
  (testing "struck-off Belize Companies Register status -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:struck-off-register} (-> (store/ledger db) last :basis)))
...
(deftest bts-registration-unverified-is-held-and-unoverridable
  (testing "unverified BTS (Belize Tax Service) tax registration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:bts-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
