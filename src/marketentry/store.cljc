(ns marketentry.store
  "SSoT for the Belize (BLZ) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:struck-off?` / `:requires-bts-registration?` /
  `:bts-registration-verified?` are ground truth for the flagship and
  conditional governor checks."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Corozal Trading Co. Ltd" :portal "procurement.gov.bz"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :struck-off? false
             :requires-bts-registration? true :bts-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BLZ" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "procurement.gov.bz"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :struck-off? false
             :requires-bts-registration? true :bts-registration-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Cayo Builders Ltd" :portal "procurement.gov.bz"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BLZ" :status :intake}
    "eng-4" {:id "eng-4" :operator "Belmopan Freight Ltd" :portal "procurement.gov.bz"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :struck-off? true
             :requires-bts-registration? true :bts-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BLZ" :status :intake}
    "eng-5" {:id "eng-5" :operator "Placencia Marine Services Ltd" :portal "procurement.gov.bz"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :struck-off? false
             :requires-bts-registration? true :bts-registration-verified? false
             :drafted? false :submitted? false
             :jurisdiction "BLZ" :status :intake}
    "eng-6" {:id "eng-6" :operator "San Ignacio Civic Tech Ltd" :portal "procurement.gov.bz"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               struck-off?
                               requires-bts-registration? bts-registration-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? struck-off?)                   (assoc :engagement/struck-off? struck-off?)
    (some? requires-bts-registration?)    (assoc :engagement/requires-bts-registration? requires-bts-registration?)
    (some? bts-registration-verified?)    (assoc :engagement/bts-registration-verified? bts-registration-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/struck-off?
   :engagement/requires-bts-registration? :engagement/bts-registration-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :struck-off? (boolean (:engagement/struck-off? m))
     :requires-bts-registration? (boolean (:engagement/requires-bts-registration? m))
     :bts-registration-verified? (boolean (:engagement/bts-registration-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-blz"})
