(ns marketentry.marketentryllm
  ...
  the Belize (BLZ) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "bts-registration-verified?=" (:bts-registration-verified? e)
                        " struck-off?=" (:struck-off? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-bts-registration? e))
                              (:bts-registration-verified? e)))
                   0.9 0.3)}))
