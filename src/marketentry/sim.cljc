...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (BLZ, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up struck-off) ==")
    ...
    (println "== filing/submit eng-4 (struck-off Companies Register -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up bts-registration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (bts-registration-unverified -> HARD hold) ==")
